...
const emailRegex = /^[\w\.]{1,64}@([\w]+\.)+[\w]{2,4}$/;
...
const userValidation = (data, strict = []) => {
	let schema = Joi.object({
		...
		email: Joi.string().trim().regex(emailRegex).messages({
			"string.pattern.base": `The local part of the mail can contain capital and small Latin letters (A-Z, a-z), numbers (from 0 to 9), but no more than 64 characters`,
		}),
		password: Joi.string()
			.trim()
			.min(6)
